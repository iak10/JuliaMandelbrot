...
class ApplicationJOrbit extends JFrame {
	...
	private int xMargin = 170; // gap between the left side of JFrame and the plotted image
	private int yMargin = 120;  // gap between lower edge of plotted image and botto of the JFrame
...
//					fileCounter++;
	...
	class Transaction implements ActionListener
	{
		...
		//	    	animationStart = true;
			    	...
			    	try {				
			    		textOut.write("Trace: Z = " + nextPoint[0] + " " + nextPoint[1] + " i " + new Date());
			    		textOut.newLine();
					} catch (IOException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
			...
			while( true )
			{
				...
				{				
					...
					if (traceRunning)
					{
						...
						newXPixel = (int) Math.round(((nextPoint[0] - minX)*(W/(maxX - minX))));
						newYPixel = (int) Math.round(((maxY - nextPoint[1])*(H/(maxY - minY))));
						//repaint(xMargin + Math.min(oldXPixel, newXPixel)-3,topOffset + Math.min(oldYPixel, newYPixel)-3,Math.abs(newXPixel-oldXPixel)+7,Math.abs(newYPixel-oldYPixel)+7);
					    repaint(xMargin + oldXPixel - 3, topOffset + oldYPixel - 3, 7, 7);
					    Thread.sleep(40);
					    repaint(xMargin + newXPixel - 3, topOffset + newYPixel - 3, 7, 7); 
					}
	...
	public void reDraw( Graphics2D g )
	{
		...
		if(traceRunning) 
		{
			plotPoint(g, newXPixel, newYPixel);
			System.out.println("plotpoint invoked x = " + newXPixel + " y = " + newYPixel);
		}
	}
	...
	public double[] nextPoint(double x, double y, double a, double b)
	{
		...
		// stop the animation if the point goes off the Julia plot
	...
	{ // check if point lies on the Julia plot
