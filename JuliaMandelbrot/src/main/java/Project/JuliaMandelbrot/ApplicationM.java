...
public class ApplicationM extends JFrame {
	...
	private double minXDefault = -2.5; // minimum value of x in normal zoom state
	private double maxXDefault =  1.0; // maximum value of x in normal zoom state
	private double minYDefault = -1.5; // minimum value of y in normal zoom state
	private double maxYDefault = 1.5;  // maximum value of y in normal zoom state			
	        ...
	        public void mouseReleased (MouseEvent e)
	        {
	        	if(!zoomSelect && xPixelArrayList.size() > 6)
	        	{
	        		...
	        		for(int i = 0; i < xPixelArrayList.size(); i++)
	        		{
	        			...
	        			xArray[i] = ((minX)+((xPixelArray[i] - xMargin)*((maxX - minX)/W)));
	        			yArray[i] = ((maxY)-((yPixelArray[i] - topOffset)*((maxY - minY)/H)));	
	        		}
...
//		System.out.println(i);
		...
		repaint(Math.min(animationX, prevAnimationX)-3,Math.min(animationY, prevAnimationY)-3,Math.abs(animationX-prevAnimationX)+7,Math.abs(animationY-prevAnimationY)+7);
	}
